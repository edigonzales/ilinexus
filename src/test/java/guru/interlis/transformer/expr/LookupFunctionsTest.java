...
class LookupFunctionsTest {
    ...
    void lookupDoesNotWarnWhenAmbiguousHitsReturnSameValue() {
        ...
        index.index(sourceRecord("DM01.BB.Gebaeudenummer", "child-1", "Gebaeudenummer_von", "2618", "GWR_EGID", "1319100"));
        index.index(sourceRecord("DM01.BB.Gebaeudenummer", "child-2", "Gebaeudenummer_von", "2618", "GWR_EGID", "1319100"));

        DiagnosticCollector diagnostics = new DiagnosticCollector();
        EvalContext ctx = new EvalContext(Map.of(), diagnostics, "test").withLookupIndex(index);

        Value value = new ExpressionEngine().evaluate(
                "lookup('DM01.BB.Gebaeudenummer', 'Gebaeudenummer_von', '2618', 'GWR_EGID')",
                ctx);
    ...
    void lookupWarnsWhenAmbiguousHitsReturnDifferentValues() {
        InMemorySourceLookupIndex index = new InMemorySourceLookupIndex();
        index.index(sourceRecord("DM01.BB.Gebaeudenummer", "child-1", "Gebaeudenummer_von", "2618", "GWR_EGID", "1319100"));
        index.index(sourceRecord("DM01.BB.Gebaeudenummer", "child-2", "Gebaeudenummer_von", "2618", "GWR_EGID", "1319101"));
        ...
        assertThat(diagnostics.all())
                .extracting(d -> d.code())
                .containsExactly(DiagnosticCode.LOOKUP_AMBIGUOUS);
    }

    private static SourceRecord sourceRecord(String tag, String oid,
                                             String refAttr, String refOid,
